...
def main_window():
    ...
    main_layout = [
        [sg.Menu(menu_def)],
        [sg.Text('備品管理アプリ',justification='c')],
        [sg.Button('新規登録', key='-Move_registration_window-', disabled=True, tooltip='備品の新規登録'),
        sg.Button('備品管理', key='-Move_change_window-', disabled=True, tooltip='備品の変更/削除')],
    ]

    return sg.Window('メイン画面', main_layout, finalize=True)
